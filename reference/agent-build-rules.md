...
- 内容主区：中文/英文无轨的章 **y 110–620**、有轨的章 **y 175–620**；俄文无轨的章 **y 110–600**、有轨的章 **y 175–600**（x 60–1220）。哪些章有轨以分镜表「常驻层」表为准。字幕带：中文/英文 **y637–690**，俄文 **y615–686**；这些区域不放任何需要阅读的内容。进度条 y687–720 只允许全幅背景/大图形穿过（会被条体提亮，这是原片风格，允许）。
- 最小字号 22px；正文标签 26–34px；标题 44–72px；英文技术词用 FONT_TECH（斜体，`scaleX 0.8–0.9`）。
- **英文或俄文片（`lang: 'en' | 'ru'`）**：拉丁与西里尔字母不要 `scaleX` 压窄（用 `SQUEEZE`，中文 .85，其它语言 1）；居中文字 `dy` 用 `TEXT_DY`；估宽用 `textW()`。细则 `style-guide.md` §3.1–3.2。
...
- **持续动作**（composition-and-light.md §7）：每个字幕块的动词有持续到下一拍的动作；入场后不许完全静止 >30 帧；没有其它运镜的镜头加 1.0→1.05 慢推（中文/英文内容留在 x 89–1191 / y 122–607；俄文留在 x 89–1191 / y 122–587）。完工前 `python3 scripts/motion_check.py <Gn>`：每镜头静止 ≤40%、最长 ≤0.7 s（成片复测口径更严），数字写进 BUILD_NOTES。
