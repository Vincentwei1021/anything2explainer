...
> 英文或俄文片（`config.ts` 的 `lang: 'en' | 'ru'`）另加两条：字幕折成两行（说明该块超出语言预算：英文 48 字符、俄文 40 字符，且已压进内容区）→ 中；进度条章名或章节卡标题被自动缩字号到发虚 → 低，回去把章名改短。俄文还需检查西里尔字母、`ё` 和标点是否正确显示。

## 看什么（按优先级）
1. **可读性/遮挡**：任何文字被字幕带（中文/英文 y637–690；俄文 y615–686）、进度条（y687–720）、HUD（y28–100）、流程轨（有轨的章 y112–160，见分镜表「常驻层」）遮住或紧贴（<10px）；文字溢出画布；字号 <22px；白字压在白/浅色块上；灰字在雾底上不可读。
2. **节拍**：关键元素出现帧 vs 对应字幕块起始帧（timeline.md）：晚 >3 帧或早 >6 帧记中；整句没有对应画面变化记高。检查方法：看字幕块起始帧 −6、0、+3、+8 四张帧。
3. **事实/拼写**：画面英文、俄文与数字逐个核对调研文档；错字/繁简混用/俄文拼写与标点错误/术语不一致（同一概念两组写法不同）记中。
4. **风格一致**：颜色不在调色板（非紫/橙红/灰/白/绿勾）、字体不对（中文/俄文非 Noto、英文技术词非 Exo 2 斜体）、描边粗细（2–3px）、圆角风格、glitch 入场缺失或过度（>12 帧闪烁）、不透明黑底盖掉幕底（大面积纯黑、无星点 / 点阵）。
