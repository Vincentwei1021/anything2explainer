...
| **内容主区** | 中文/英文：无轨的章 y110–620，有轨的章 y175–620；俄文：无轨 y110–600，有轨 y175–600；x60–1220 | 关键信息只放这里；俄文字幕位置更高，需预留额外底部空间 |
| 字幕带 | 中文/英文 CSS top 637，俄文 CSS top 615；44px Noto 700 白 + 4px 黑描边（16+8+4 向 text-shadow 环）居中 | **不放任何内容**；入场轨迹不得穿过 |
...
### 3.2 俄文片（`config.ts` 的 `lang: 'ru'`）

俄文正文与字幕继续用 комплектный Noto Sans SC：основная кириллица и `ё` в шрифте есть. Как и для английского, `SQUEEZE=1` и `TEXT_DY=0`; ширина русских букв считается по таблице реальных glyph advances в `common/textfit.ts`. Бюджет одного блока субтитров — 40 знаков. Символы, отсутствующие в комплектном шрифте (например `₽`), лучше писать словами (`рублей`) либо отдельно проверять fallback-рендером.
