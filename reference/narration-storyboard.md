...
- 篇幅：**按用户定的时长取表里那一档**（如 3–5 分钟 → 40–50 句、1200–1500 字）。语速约 6 字/秒（英文见 §2.5），加 20 帧句间留白后成片密度约 4.3–4.8 字/秒（成片比纯语音长 6–8%，是停留预算）。每句 ≤35 字，长句用竖线切成 2–5 块。**短句是好文案**（节奏变化，`narration-guidance.md` §7），不必为画面避免它：分镜阶段短句并入相邻镜头当节拍，不单独成镜头。写完先估时长，超/欠 15% 就加删句子。
...
- 逐句合成 + WordBoundary → 字幕块起止帧；句间 GAP 20 帧（停留预算；第一至五片用 10–16，实测末拍刚落位就切）、章前 CHAPTER_GAP 45、片头 LEAD 40、片尾 TAIL 90；音量归一到峰值 0.89。
- `timeline.md` 多一列「末块」（末尾字幕块帧数）并在终端列出 <45 帧的句：末拍元素 22 帧入场 + 8 帧离场后停不满 30 帧。在这些句后加 `## gap 15–30`（不改词、缓存命中、重跑几秒），或分镜时把末拍元素前挂到上一块。
...
1. 头部：总帧数、章节帧、衔接规则（镜头区间 = [首句 from−8, 下一句 from−9]，章末 / 片末 = 末句 to+2；末 8 帧离场；元素入场对齐字幕块起始帧 −6…+3；末拍落位 → 离场 ≥30 帧；运镜不进末拍）。**镜头按画面单元分，不按句分**：短句（<3 s 或单块）并入相邻镜头当节拍，单镜头 ≥120 帧，能承接就不清场。
...
4. 全局约束：示例语境（问句/答复/来源文案原文）、事实清单（画面允许出现的数字/英文）、闪烁白名单（每镜头的重点词）、复用图元清单、**高光时刻清单**（每章 ≤1 个镜头：章论点 / 主角登场 / 收尾 payoff，各标登场型 / 论点型 / 大数字型 / 象征物型，按 `composition-and-light.md` §3 编排，≥90 帧）、**扫光白名单**（登场型专用，全片 ≤2 个镜头：核心概念首次登场 + 可选结尾回扣；其余镜头不得出现 `LightSweep` / `StageLine` / `GhostText`）、**运镜清单**（每章 ≥3 处，写在哪个镜头哪个节拍、哪一种）、**§9 持续动作与落位停留**（照抄 `composition-and-light.md` §7 的两条规则与量化判据：入场后不许完全静止 >3 s、不为凑动作加漂浮、末拍落位后停 30–45 帧再离场、运镜不进末拍，构建组用 `scripts/motion_check.py <Gn>` 自测）。
