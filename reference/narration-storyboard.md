...
4. 全局约束：示例语境（问句/答复/来源文案原文）、事实清单（画面允许出现的数字/英文）、闪烁白名单（每镜头的重点词）、复用图元清单、**高光时刻清单**（每章 1–2 个镜头：章论点 / 主角登场 / 收尾 payoff，各标登场型 / 大数字型 / 象征物型，按 `composition-and-light.md` §3 编排，≥90 帧）、**运镜清单**（每章 ≥3 处，写在哪个镜头哪个节拍、哪一种）、**§9 持续动作与落位停留**（照抄 `composition-and-light.md` §7 的两条规则与量化判据：入场后不许完全静止 >3 s、不为凑动作加漂浮、末拍落位后停 30–45 帧再离场、运镜不进末拍，构建组用 `scripts/motion_check.py <Gn>` 自测）。
