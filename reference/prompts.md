...
工程 <项目根>，只能改 `src/shots/<Gn>/**`（index.ts 已建好；图元 `import from '../../ui'`，光效/运镜 `from '../../fx'`，缓动/共用层 `from '../../common'`）。<英文/俄文片：`config.ts` 的 `lang: 'en' | 'ru'`——拉丁与西里尔字母不要 scaleX 压窄（用 `SQUEEZE`）、居中文字 `dy` 用默认值、估宽用 `textW()`，细则 style-guide.md §3.1–3.2。>先 `cat src/ui.tsx` 看可用图元。预览合成 id = `<Gn>`（含覆盖层）。<有流程轨的章：y112–165 有流程轨；中文/英文内容主区 y175–620，俄文 y175–600。>
