...
![样片《RAG 与知识库》前 30 秒的帧总表：黑底星空、白线条图形配紫色重点、超粗黑体大字、44px 字幕与章节进度条](examples/rag/frames/overview_1.jpg)
...
- **耗时**：按时长约 1.5–5 小时，大部分时间是 agent 并行构建镜头。全程只在四个确认点问你。
...
| 时长 | 中文字数 | 英文词数 | 句 / 镜头数 | 章节 | 构建 agent | 产出耗时 | 磁盘 |
|---|---|---|---|---|---|---|---|
| 2–3 分钟 | 700–950 | 280–420 | 24–32 | 3 | 4–6 | ≈1.5 小时 | ≈2GB |
| 3–5 分钟（样片档） | 1200–1500 | 420–700 | 40–50 | 4 | 8 | ≈3 小时 | ≈2GB |
| 5–8 分钟 | 1800–2400 | 700–1150 | 60–80 | 5–6 | 10–14 | ≈4–5 小时 | ≈3GB |
...
1. **时长与语言**：写文案之前。时长决定章节数、句数、镜头数和并行 agent 数，也就决定内容能铺多丰富；语言决定 `src/config.ts` 的 `lang`，进而影响排版、字幕预算和默认音色。
...
**能换视觉风格吗？**
只做这一种风格，是有意为之。要换就改 `reference/style-guide.md` 和 `src/ui.tsx`，镜头代码只用这些图元。
...
**支持哪些语言？**
中文和英文。各有自己的语速模型、字幕预算与默认音色。样片是中文片，英文片目前还没有对标样片，但图形语言与语言无关。
...
- 中英文都支持（`lang: 'zh' | 'en'`），各有自己的语速、字幕块预算（每块 16 字 / 48 字符）与默认音色。样片是中文片，英文片目前还没有对标样片，但图形语言与语言无关。只做这一种视觉风格，换风格要改 `reference/style-guide.md` + `src/ui.tsx`。
